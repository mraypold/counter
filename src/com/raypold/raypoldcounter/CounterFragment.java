...
public class CounterFragment extends Fragment implements View.OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		inflatedView = inflater.inflate(R.layout.fragment_counter, container,
				false);
		...
		Button increment = (Button) inflatedView
		...
		Button decrement = (Button) inflatedView
	...
	public static void setTextDisplay() {
		/*
		 * findViewById() doesn't work inside a fragment. Must use on
		 * inflatedView
		 */
		TextView counterTextView = (TextView) inflatedView
				.findViewById(R.id.counterNameView);
		counterTextView.setText(workingCounterName);
	}

	public static void displayCount() {
		String currentCount = String.valueOf(openCounter.getCurrentCount());
		TextView countTextView = (TextView) inflatedView
				.findViewById(R.id.countDisplay);
		countTextView.setText(currentCount);
	}

	public static void refreshDisplay() {
		/*
		 * Not an ideal solution since high I/O overhead, but ensures that the
		 * counter stays in sync with changes from the actionBar and other
		 * fragment tabs.
		 * 
		 * If redesigning the application, I would keep an open counter in the
		 * MainActivity and then have getters and setters to ensure all
		 * fragments are using the same counter.
		 */
